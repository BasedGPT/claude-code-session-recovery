...
- No clipboard access
- Session tools spawn only their documented local helpers: Windows `wmic`,
  PowerShell, and `tasklist` process probes; macOS `pgrep`/`ps` probes;
  `claude --version` for version detection; and PowerShell plus `vssadmin` for
  `restore_from_vss.py`. Worktree tools invoke Git as documented in the
  lifecycle guide.
